...
namespace CLabs.Tickets.Linq
{
    ...
    {
        public static ITicketAsyncEnumerable<IList<TSource>> Buffer<TSource>(this ITicketAsyncEnumerable<TSource> source, Int32 count)
        {
        ...
        public static ITicketAsyncEnumerable<IList<TSource>> Buffer<TSource>(this ITicketAsyncEnumerable<TSource> source, Int32 count, Int32 skip)
        {
            Error.ThrowArgumentNullException(source, nameof(source));
            if (count <= 0) throw Error.ArgumentOutOfRange(nameof(count));
    ...
    internal sealed class Buffer<TSource> : ITicketAsyncEnumerable<IList<TSource>>
    {
        readonly ITicketAsyncEnumerable<TSource> source;
        readonly int count;

        public Buffer(ITicketAsyncEnumerable<TSource> source, int count)
        {
            this.source = source;
            this.count = count;
        }

        public ITicketAsyncEnumerator<IList<TSource>> GetAsyncEnumerator(CancellationToken cancellationToken = default)
        {
            return new _Buffer(source, count, cancellationToken);
        }

        sealed class _Buffer : MoveNextSource, ITicketAsyncEnumerator<IList<TSource>>
        {
            ...
            public _Buffer(ITicketAsyncEnumerable<TSource> source, int count, CancellationToken cancellationToken)
            {
                this.source = source;
                this.count = count;
    ...
    internal sealed class BufferSkip<TSource> : ITicketAsyncEnumerable<IList<TSource>>
    {
        readonly ITicketAsyncEnumerable<TSource> source;
        readonly int count;
        readonly int skip;

        public BufferSkip(ITicketAsyncEnumerable<TSource> source, int count, int skip)
        {
            this.source = source;
            this.count = count;
            this.skip = skip;
        ...
        sealed class _BufferSkip : MoveNextSource, ITicketAsyncEnumerator<IList<TSource>>
        {
            static readonly Action<object> MoveNextCoreDelegate = MoveNextCore;
            ...
            public _BufferSkip(ITicketAsyncEnumerable<TSource> source, int count, int skip, CancellationToken cancellationToken)
            {
                this.source = source;
                this.count = count;
                this.skip = skip;
