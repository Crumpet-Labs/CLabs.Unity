...
namespace CLabs.Tickets.Internal
{
    internal static class DiagnosticsExtensions
    {
        ...
        static string BeautifyType(Type t, bool shortName)
        {
            ...
            if (!t.IsGenericType) return shortName ? t.Name : t.FullName.Replace("CLabs.Tickets.Triggers.", "").Replace("CLabs.Tickets.Internal.", "").Replace("CLabs.Tickets.", "") ?? t.Name;
            ...
            return typeBeautifyRegex.Replace(genericType, "").Replace("CLabs.Tickets.Triggers.", "").Replace("CLabs.Tickets.Internal.", "").Replace("CLabs.Tickets.", "") + "<" + innerFormat + ">";
        }
        ...
        static bool IgnoreLine(MethodBase methodInfo)
        {
            ...
            else if (declareType.StartsWith("CLabs.Tickets.CompilerServices"))
            {
                return true;
            }
            ...
            else if (declareType.StartsWith("CLabs.Tickets.TicketCompletionSourceCore"))
            {
                return true;
            }
            else if (declareType.StartsWith("CLabs.Tickets.AwaiterActions"))
            {
                return true;
            }
        ...
        static string AppendHyperLink(string path, string line)
        {
            ...
            {
                var fname = fi.FullName.Replace(Path.DirectorySeparatorChar, '/').Replace(TicketRuntime.ApplicationDataPath ?? "", "");
