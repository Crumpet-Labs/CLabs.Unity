...
namespace CLabs.Tickets
{
    ...
    public abstract class ChannelReader<T>
    {
        ...
        public abstract Ticket<bool> WaitToReadAsync(CancellationToken cancellationToken = default(CancellationToken));

        public abstract Ticket Completion { get; }

        public virtual Ticket<T> ReadAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            if (this.TryRead(out var item))
            {
                return Ticket.FromResult(item);
            }

            return ReadAsyncCore(cancellationToken);
        }

        async Ticket<T> ReadAsyncCore(CancellationToken cancellationToken = default(CancellationToken))
        {
        ...
        public abstract ITicketAsyncEnumerable<T> ReadAllAsync(CancellationToken cancellationToken = default(CancellationToken));
    }
    ...
    internal sealed class SingleConsumerUnboundedChannel<T> : Channel<T>
    {
        ...
        TicketCompletionSource completedTaskSource;
        Ticket completedTask;
                ...
                lock (parent.items)
                {
                    ...
                    if (parent.items.Count == 0)
                    {
                        if (error == null)
                        {
                            ...
                            {
                                parent.completedTask = Ticket.CompletedTask;
                            }
                        }
                        else
                        {
                            ...
                            {
                                parent.completedTask = Ticket.FromException(error);
                            }
                        }
        ...
        sealed class SingleConsumerUnboundedChannelReader : ChannelReader<T>, ITicketSource<bool>
        {
            ...
            TicketCompletionSourceCore<bool> core;
            ...
            public override Ticket Completion
            {
                get
                {
                    ...
                    parent.completedTaskSource = new TicketCompletionSource();
                    ...
                    if (parent.items.Count != 0)
                    {
                        ...
                        if (parent.closed && parent.items.Count == 0)
                        {
                            if (parent.completionError != null)
                            {
                                ...
                                {
                                    parent.completedTask = Ticket.FromException(parent.completionError);
                                }
                            }
                            else
                            {
            ...
            public override Ticket<bool> WaitToReadAsync(CancellationToken cancellationToken)
            {
                ...
                lock (parent.items)
                {
                    ...
                    if (parent.closed)
                    {
                        ...
                        {
                            return Ticket.FromException<bool>(parent.completionError);
                        }
                    }
                    ...
                    return new Ticket<bool>(this, core.Version);
                }
            }
            ...
            public override ITicketAsyncEnumerable<T> ReadAllAsync(CancellationToken cancellationToken = default)
            {
                return new ReadAllAsyncEnumerable(this, cancellationToken);
            }

            bool ITicketSource<bool>.GetResult(short token)
            {
            ...
            sealed class ReadAllAsyncEnumerable : ITicketAsyncEnumerable<T>, ITicketAsyncEnumerator<T>
            {
