...
{
    public static class TicketObservableExtensions
    {
        public static Ticket<T> ToTicket<T>(this IObservable<T> source, bool useFirstValue = false, CancellationToken cancellationToken = default)
        {
            var promise = new TicketCompletionSource<T>();
            var disposable = new SingleAssignmentDisposable();

            var observer = useFirstValue
                ? (IObserver<T>)new FirstValueToTicketObserver<T>(promise, disposable, cancellationToken)
                : (IObserver<T>)new ToTicketObserver<T>(promise, disposable, cancellationToken);
        ...
        public static IObservable<T> ToObservable<T>(this Ticket<T> task)
        {
        ...
        /// <summary>
        /// Ideally returns IObservabl[Unit] is best but CLabs.Tickets does not have Unit so return AsyncUnit instead.
        /// </summary>
        public static IObservable<AsyncUnit> ToObservable(this Ticket task)
        {
            if (task.Status.IsCompleted())
            {
                try
        ...
        static async TicketVoid Fire<T>(AsyncSubject<T> subject, Ticket<T> task)
        {
            T value;
            try
            {
        ...
        static async TicketVoid Fire(AsyncSubject<AsyncUnit> subject, Ticket task)
        {
            try
            {
                await task;
        ...
        class ToTicketObserver<T> : IObserver<T>
        {
            static readonly Action<object> callback = OnCanceled;

            readonly TicketCompletionSource<T> promise;
            ...
            public ToTicketObserver(TicketCompletionSource<T> promise, SingleAssignmentDisposable disposable, CancellationToken cancellationToken)
            {
            ...
            {
                var self = (ToTicketObserver<T>)state;
        ...
        class FirstValueToTicketObserver<T> : IObserver<T>
        {
            static readonly Action<object> callback = OnCanceled;
            ...
            public FirstValueToTicketObserver(TicketCompletionSource<T> promise, SingleAssignmentDisposable disposable, CancellationToken cancellationToken)
            {
                this.promise = promise;
                this.disposable = disposable;
                this.cancellationToken = cancellationToken;
            ...
            static void OnCanceled(object state)
            {
                var self = (FirstValueToTicketObserver<T>)state;
                self.disposable.Dispose();
                self.promise.TrySetCanceled(self.cancellationToken);
            }
