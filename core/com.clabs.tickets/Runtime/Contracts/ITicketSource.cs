...
#if (Ticket_NETCORE && !NETSTANDARD2_0) || UNITY_2022_3_OR_NEWER
...
namespace CLabs.Tickets
{
...
#endif
    {
        ...
        TicketStatus UnsafeGetStatus(); // only for debug use.

#if SUPPORT_VALUETASK

        System.Threading.Tasks.Sources.ValueTaskSourceStatus System.Threading.Tasks.Sources.IValueTaskSource.GetStatus(short token)
        {
            return (System.Threading.Tasks.Sources.ValueTaskSourceStatus)(int)((ITicketSource)this).GetStatus(token);
        }

        void System.Threading.Tasks.Sources.IValueTaskSource.GetResult(short token)
        {
            ((ITicketSource)this).GetResult(token);
        }
        ...
        void System.Threading.Tasks.Sources.IValueTaskSource.OnCompleted(Action<object> continuation, object state, short token, System.Threading.Tasks.Sources.ValueTaskSourceOnCompletedFlags flags)
        {
            ...
            ((ITicketSource)this).OnCompleted(continuation, state, token);
        }

#endif
    }

    public interface ITicketSource<out T> : ITicketSource
#if SUPPORT_VALUETASK
...
#endif
    {
        ...
        new public TicketStatus GetStatus(short token)
        {
            return ((ITicketSource)this).GetStatus(token);
        }
        ...
        {
            return ((ITicketSource<T>)this).GetResult(token);
        }
    ...
    public static class TicketStatusExtensions
    {
        ...
        public static bool IsCompleted(this TicketStatus status)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool IsCompletedSuccessfully(this TicketStatus status)
        {
            return status == TicketStatus.Succeeded;
        }

        /// <summary>status == Canceled.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool IsCanceled(this TicketStatus status)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool IsFaulted(this TicketStatus status)
        {
            return status == TicketStatus.Faulted;
        }
    }
}
