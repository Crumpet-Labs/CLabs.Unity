...
{
    public sealed class TicketTrackerViewItem : TreeViewItem
    {
        ...
        public TicketTrackerViewItem(int id) : base(id)
        {

        }
    }

    public sealed class TicketTrackerTreeView : TreeView
    {
        const string sortedColumnIndexStateKey = "TicketTrackerTreeView_sortedColumnIndex";

        public IReadOnlyList<TreeViewItem> CurrentBindingItems;

        public TicketTrackerTreeView()
        ...
        TicketTrackerTreeView(TreeViewState state, MultiColumnHeader header)
            : base(state, header)
        {
            rowHeight = 20;
        ...
        private void Header_sortingChanged(MultiColumnHeader multiColumnHeader)
        {
            ...
            var items = rootItem.children.Cast<TicketTrackerViewItem>();

            IOrderedEnumerable<TicketTrackerViewItem> orderedEnumerable;
            switch (index)
            {
                case 0:
        ...
        protected override TreeViewItem BuildRoot()
        {
            ...
            {
                children.Add(new TicketTrackerViewItem(trackingId) { TaskType = awaiterType, Status = status.ToString(), Elapsed = (DateTime.UtcNow - created).TotalSeconds.ToString("00.00"), Position = stackTrace });
            });
        ...
        {
            var item = args.item as TicketTrackerViewItem;
