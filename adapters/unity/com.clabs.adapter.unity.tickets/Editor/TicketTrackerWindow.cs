...
namespace CLabs.Tickets.Unity.Editor
{
    public sealed class TicketTrackerWindow : EditorWindow
    {
        static int interval;

        static TicketTrackerWindow window;

        [MenuItem("Window/Ticket Tracker")]
        public static void OpenWindow()
        {
            ...
            GetWindow<TicketTrackerWindow>("Ticket Tracker").Show();
        }

        static readonly GUILayoutOption[] EmptyLayoutOption = new GUILayoutOption[0];

        TicketTrackerTreeView treeView;
        ...
        void OnEnable()
        {
            ...
            treeView = new TicketTrackerTreeView();
        ...
        static readonly GUIContent EnableTrackingHeadContent = EditorGUIUtility.TrTextContent("Enable Tracking", "Start to track async/await Ticket. Performance impact: low", (Texture)null);
        ...
        void RenderHeadPanel()
        {
            ...
            {
                var newValue = !EnableAutoReload;
                TaskTracker.EditorEnableState.EnableAutoReload = newValue;
                EditorPrefs.SetBool(TaskTracker.EnableAutoReloadKey, newValue);
            }

            if (GUILayout.Toggle(EnableTracking, EnableTrackingHeadContent, EditorStyles.toolbarButton, EmptyLayoutOption) != EnableTracking)
            {
                var newValue = !EnableTracking;
                TaskTracker.EditorEnableState.EnableTracking = newValue;
                EditorPrefs.SetBool(TaskTracker.EnableTrackingKey, newValue);
            }
            ...
            {
                var newValue = !EnableStackTrace;
                TaskTracker.EditorEnableState.EnableStackTrace = newValue;
                EditorPrefs.SetBool(TaskTracker.EnableStackTraceKey, newValue);
            }
        ...
        void RenderDetailsPanel()
        {
            ...
            if (selected.Count > 0)
            {
                ...
                var item = treeView.CurrentBindingItems.FirstOrDefault(x => x.id == first) as TicketTrackerViewItem;
                if (item != null)
                {
