...
namespace CLabs.Tickets
{
    ...
    {
        public static async Ticket WaitAsync(this JobHandle jobHandle, PlayerLoopTiming waitTiming, CancellationToken cancellationToken = default)
        {
            await Ticket.Yield(waitTiming);
            jobHandle.Complete();
            cancellationToken.ThrowIfCancellationRequested(); // call cancel after Complete.
        }

        public static Ticket.Awaiter GetAwaiter(this JobHandle jobHandle)
        {
            ...
            return new Ticket(handler, token).GetAwaiter();
        }

        // can not pass CancellationToken because can't handle JobHandle's Complete and NativeArray.Dispose.

        public static Ticket ToTicket(this JobHandle jobHandle, PlayerLoopTiming waitTiming)
        {
            var handler = JobHandlePromise.Create(jobHandle, out var token);
            {
            ...
            return new Ticket(handler, token);
        }

        sealed class JobHandlePromise : ITicketSource, IPlayerLoopItem
        {
            JobHandle jobHandle;
