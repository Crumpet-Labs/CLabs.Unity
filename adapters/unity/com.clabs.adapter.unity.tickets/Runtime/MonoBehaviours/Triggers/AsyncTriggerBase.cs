...
{
    public abstract class AsyncTriggerBase<T> : MonoBehaviour, ITicketAsyncEnumerable<T>
    {
        ...
        sealed class AsyncTriggerEnumerator : MoveNextSource, ITicketAsyncEnumerator<T>, ITriggerHandler<T>
        {
    ...
    {
        Ticket IAsyncOneShotTrigger.OneShotAsync()
        {
            core.Reset();
            return new Ticket((ITicketSource)this, core.Version);
        }
    }

    public sealed partial class AsyncTriggerHandler<T> : ITicketSource<T>, ITriggerHandler<T>, IDisposable
    {
        static Action<object> cancellationCallback = CancellationCallback;
