...
namespace CLabs.Tickets.Internal
{
    ...
    {
        // Runs before scene load so TicketEqualityComparer.GetDefault<T>()
        // immediately starts returning Unity-aware comparers (float-fuzzy
        // equality for Vector3, Color, Quaternion, etc.) instead of the
        // package's EqualityComparer<T>.Default fallback. This is how the
        // engine-agnostic tickets package picks up the Unity-specific
        // comparison semantics without taking a direct dependency on
        // anything in this adapter assembly.
        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.BeforeSceneLoad)]
        static void RegisterWithTickets()
        {
            TicketEqualityComparer.SetOverride(GetDefaultHelper);
        }
        ...
        internal static object GetDefaultHelper(Type type)
        {
