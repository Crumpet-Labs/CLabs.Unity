...
Unity bindings for `com.clabs.fork`. Roots Fork's save directory at `Application.persistentDataPath`, ships an application loader that wires the package with the Unity defaults, and provides a save-slot viewer editor window.
...
3. Assign the configuration SO to the loader's `m_Configuration` field (optional — leaving it empty falls back to the core package's `DefaultForkConfiguration`, which writes to a relative `"Saves"` directory).
...
public override Awaitable LoadAsync(CancellationToken cancellationToken) {
    ...
    builder.UseForkPackage()
        .WithFactory<IForkConfiguration>(() => m_ForkConfig);
    // ... other packages ...
    m_App = builder.Build();
...
`UseForkPackage` registers `IForkConfiguration` via the single-arg `AddSingleton<T>().WithFactory(...)` form, so the dictionary key inside `IConfigurableCollection` is the interface — which is what `WithFactory<IForkConfiguration>` looks up.

## Dependencies

- `com.clabs.fork` — core save-slot manager
...
- The `com.clabs.fork` package's `Documentation~/Guide.md` — full save-slot walkthrough.
