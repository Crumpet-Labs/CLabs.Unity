...
namespace CLabs.Fork.Samples {
    /// <summary>
    /// Drives three save slots ("forks"). Wire the shipped CLabs/Fork/Application Loader into your
    /// app so IForkService is injected here, then drive from the inspector context menu.
    /// </summary>
    public sealed partial class SaveBench : MonoBehaviour {
        ...
        public async void Save() {
            ...
            var result = await i_Fork.SaveAsync(m_SlotId, save);
        ...
        public async void Load() {
            var result = await i_Fork.LoadAsync<ChefSave>(m_SlotId);
            if (result.Success)
        ...
        public async void DeleteSlot() {
            var deleted = await i_Fork.DeleteSlotAsync(m_SlotId);
