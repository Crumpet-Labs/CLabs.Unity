...
Both stations log independently: one `RingBell` fanning out to two subscribers.
...
- `OnBell<CrumpetReady>(…)` returns an `IDisposable`. Dispose it in `OnDisable`, as shown.
